import { Camera, Upload, Sofa, Search, Sparkles, Scissors } from 'lucide-react';
...
interface PermissionGuideProps {
  ...
  onUseRoomMode?: () => void;
  onUseAISearch?: () => void;
  onUseImagePicker?: () => void;
}

export function PermissionGuide({ onRequestPermission, onUseUploadMode, onUseRoomMode, onUseAISearch, onUseImagePicker }: PermissionGuideProps) {
  return (
    <div className="flex items-start sm:items-center justify-center h-full p-4 overflow-y-auto">
      <div className="w-full max-w-md">
        ...
        {onUseImagePicker && (
          <button
            onClick={onUseImagePicker}
            className="w-full py-3.5 rounded-2xl font-medium text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.97] mb-4"
            style={{
              background: 'linear-gradient(135deg, var(--ar-accent-2-glow), var(--ar-accent-glow))',
              color: 'var(--ar-text)',
              border: '1px solid var(--ar-border)',
            }}
          >
            <Scissors className="w-5 h-5" style={{ color: 'var(--ar-accent-2)' }} />
            画像からアイテムを切り抜いて装着
          </button>
        )}
      </div>
