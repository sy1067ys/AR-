...
import {
  ...
  ShoppingBag, Sparkles, SwitchCamera, Scan, Home,
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    try { const s = localStorage.getItem(CUSTOM_ITEMS_STORAGE_KEY); if (s) setCustomItems(JSON.parse(s)); } catch {}
  }, []);
  useEffect(() => {
  ...
  const handleDownload = () => {
    ...
    const a = document.createElement('a'); a.href = capturedImage; a.download = `ar-tryOn-${Date.now()}.png`; a.click();
  };
  ...
  // AI search from home: open camera in environment mode, then immediately show AI capture
  const startAISearch = () => {
    setMode('camera');
    setFacingMode('environment');
    setHasStarted(true);
    // Small delay to let camera initialize, then hint user
    setTimeout(() => doAICapture(), 1500);
  };
  ...
  return (
    <div className="size-full flex flex-col overflow-hidden" style={{ background: 'var(--ar-bg)' }}>
      {/* ===== HEADER ===== */}
      <header
        className="flex-shrink-0 px-3 py-2.5 sm:px-5 sm:py-3 flex items-center justify-between gap-2"
        style={{ background: 'var(--ar-surface)', borderBottom: '1px solid var(--ar-border)' }}
      >
        <div className="flex items-center gap-2">
          {/* Home / Back button when active */}
          {hasStarted && (
            <button
              onClick={goHome}
          ...
          <div className="flex items-center p-0.5 rounded-xl gap-0.5" style={{ background: 'var(--ar-surface-2)' }}>
            {modeButtons.map(({ key, icon: Icon, label }) => (
              <button
                key={key}
                onClick={() => {
                  setMode(key); setCapturedImage(null);
        ...
        {!hasStarted ? (
          ...
          <>
            ...
            <button
              ...
              className="sm:hidden fixed bottom-5 right-4 z-30 w-12 h-12 rounded-full flex items-center justify-center shadow-xl active:scale-90 transition-transform"
            ...
            <div className={`sm:w-72 sm:relative sm:block fixed inset-x-0 bottom-0 z-20 sm:z-auto transition-transform duration-300 ease-out ${showRoomSelector ? 'translate-y-0' : 'translate-y-full sm:translate-y-0'} max-h-[60vh] sm:max-h-none overflow-hidden rounded-t-2xl sm:rounded-2xl`} style={{ border: '1px solid var(--ar-border)' }}>
                ...
                <>
                  ...
                  {mode === 'camera' && (
                    <div className="absolute top-3 right-3 flex flex-col gap-2 z-10">
                      ...
                      <button
                        onClick={doAICapture}
                        className="w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md active:scale-90 transition-all"
                        style={{ background: 'linear-gradient(135deg, rgba(66,133,244,0.3), rgba(52,168,83,0.3))', border: '1px solid rgba(66,133,244,0.3)', color: '#fff' }}
                      >
                  ...
                  <div className="absolute bottom-4 left-0 right-0 flex items-center justify-center gap-3 px-4 z-10">
                    <button
                      ...
                      className="w-11 h-11 rounded-full flex items-center justify-center backdrop-blur-md active:scale-90 transition-all"
                      style={{ background: showSelector ? 'var(--ar-accent)' : 'var(--ar-glass)', border: '1px solid var(--ar-glass-border)', color: showSelector ? '#fff' : 'var(--ar-text-2)' }}
                    >
                    ...
                    <button
                      ...
                      style={{
                        background: mode === 'camera' ? 'linear-gradient(135deg, var(--ar-accent), var(--ar-accent-2))' : 'linear-gradient(135deg, var(--ar-success), #45B7D1)',
                    ...
                    {mode === 'camera' ? (
                      <button
                        onClick={doAICapture}
                        className="w-11 h-11 rounded-full flex items-center justify-center backdrop-blur-md active:scale-90 transition-all"
                        style={{ background: 'linear-gradient(135deg, rgba(66,133,244,0.25), rgba(52,168,83,0.25))', border: '1px solid rgba(66,133,244,0.2)', color: '#fff' }}
                      >
                        <Scan className="w-4 h-4" />
                      </button>
                    ) : (
